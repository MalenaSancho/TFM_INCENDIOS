...
print("Fase E.2: Iniciando integración espaciotemporal multivariante para observaciones de control (Y=0)...")

# --- 1. CONFIGURACIÓN DE RUTAS Y CARGA DE DATOS SINTÉTICOS ---
ruta_script = os.path.dirname(os.path.abspath(__file__))
# Carga de la base de datos de pseudo-ausencias geolocalizadas
ruta_base = os.path.join(ruta_script, '..', 'DATOS', '1.INCENDIOS', 'BASE_NO_INCENDIOS_LATLON.csv')

df = pd.read_csv(ruta_base)
df['Fecha'] = pd.to_datetime(df['Fecha'])
print(f"Cargadas {len(df)} observaciones de control (Clase Y=0).")
...
def extraer_datos_satelite(row):
    ...
    try:
        ...
        # Detección de compresión (Magic Bytes de formato ZIP)
...
print("Ejecutando cruce de variables multivariante para la clase Y=0. Este proceso requiere alta capacidad de cómputo...")
df_nuevas = df.apply(extraer_datos_satelite, axis=1)
df_final = pd.concat([df, df_nuevas], axis=1)

# --- 4. EXPORTACIÓN ---
ruta_salida = os.path.join(ruta_script, '..', 'DATOS', 'DATASET_NO_INCENDIOS_CRUZADO.csv')
df_final.to_csv(ruta_salida, index=False)

print("-" * 50)
print("Integración espaciotemporal completada.")
print(f"Matriz de control consolidada exportada en:\n{ruta_salida}")
print("-" * 50)